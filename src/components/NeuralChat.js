...
Give them amazing movie suggestions that match what they want!`;
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const botResponse = await callMovieAPI(userMessage);
      ...
      // Speak the response if voice assistant is initialized
      if (voiceAssistantRef.current) {
        // Remove emojis and markdown for speech
        const cleanText = botResponse.replace(/[^\x00-\x7F]/g, "").replace(/\*\*/g, "");
        voiceAssistantRef.current.speak(cleanText);
      }
                    ...
                    <button
                      onClick={() => {
                        if (isListening) {
                          ...
                          voiceAssistantRef.current?.startListening((transcript) => {
                            setInput(transcript);
                      ...
                      className={`px-4 py-2 rounded-xl font-semibold transition-all duration-300 ${
                        isListening
                          ? "bg-red-600 border border-red-400/60"
