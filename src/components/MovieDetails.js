...
import { generateMovieSpoiler, getCachedSpoiler, cacheSpoiler } from '../utils/geminiApi';
...
const MovieDetails = () => {
  ...
  const [loadingSpoiler, setLoadingSpoiler] = useState(false);
  ...
  const handleSpoilerClick = async () => {
    ...
    // Check cache first
    const cached = getCachedSpoiler(movie.id);
    if (cached) {
      setSpoilerContent(cached);
      return;
    }
    
    // Generate new spoiler
    setLoadingSpoiler(true);
    try {
      const generatedSpoiler = await generateMovieSpoiler(movie);
      setSpoilerContent(generatedSpoiler);
      cacheSpoiler(movie.id, generatedSpoiler);
    } catch (error) {
      console.error('Error generating spoiler:', error);
      setSpoilerContent(movie.overview || 'Unable to generate spoiler at this time.');
    } finally {
      setLoadingSpoiler(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-black text-white pt-28 md:pt-32">
    ...
    <div className="min-h-screen bg-black text-white">
            ...
            <div className="lg:col-span-2 space-y-4 sm:space-y-6 order-2 lg:order-2">
              ...
              <div>
                ...
                <button
                  ...
                  disabled={loadingSpoiler}
                  className="mt-4 bg-yellow-600/20 border-2 border-yellow-500/50 text-yellow-300 hover:bg-yellow-600/30 hover:border-yellow-400 px-6 py-3 rounded-lg font-['JetBrains_Mono',monospace] font-bold flex items-center gap-2 transition-all duration-300 transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {loadingSpoiler ? (
                    <>
      ...
      {movie && (
        <DescriptionModal
          ...
          title={movie.title}
          overview={spoilerContent || movie.overview}
          fullDescription={spoilerContent || movie.overview}
